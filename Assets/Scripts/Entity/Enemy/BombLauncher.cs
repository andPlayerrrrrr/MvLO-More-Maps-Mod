...
public class BombLauncher : MonoBehaviourPun {
    ...
    void TryToShoot() {
        ...
        if(right)
        {
            if (IntersectsPlayer((Vector2)transform.position + searchOffset, searchBox))
            {
                GameObject newBill = PhotonNetwork.InstantiateRoomObject("Prefabs/Enemy/Bobomb", transform.position + new Vector3(spawnOffset.x, spawnOffset.y), Quaternion.identity, 0, new object[] { false });
                bills.Add(newBill);
                return;
            }
        }
