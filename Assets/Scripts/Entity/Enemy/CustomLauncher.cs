...
public class CustomLauncher : MonoBehaviourPun
{
    ...
    void TryToShoot()
    {
        ...
        if (left)
        {
            ...
            {
                GameObject newBill = PhotonNetwork.InstantiateRoomObject("Prefabs/" + other, transform.position + new Vector3(-spawnOffset.x, spawnOffset.y), Quaternion.identity, 0, new object[] { true });
                bills.Add(newBill);
                return;
            }
        }
        ...
        if (right)
        {
            if (IntersectsPlayer((Vector2)transform.position + searchOffset, searchBox))
            {
                GameObject newBill = PhotonNetwork.InstantiateRoomObject("Prefabs/" + other, transform.position + new Vector3(spawnOffset.x, spawnOffset.y), Quaternion.identity, 0, new object[] { false });
                bills.Add(newBill);
                return;
            }
        }
